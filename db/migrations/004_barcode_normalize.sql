-- Sticker sin espacios ↔ clave Excel con espacios: búsqueda normalizada en process_scan

CREATE OR REPLACE FUNCTION normalize_barcode_key(p_text TEXT)
RETURNS TEXT AS $$
BEGIN
  RETURN UPPER(REPLACE(COALESCE(p_text, ''), ' ', ''));
END;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('result', 'not_found', 'message', 'Session not found');
  END IF;
  IF v_session.status = 'closed' THEN
    RETURN jsonb_build_object('result', 'not_found', 'message', 'Session is closed');
  END IF;
  ...
  IF NOT FOUND THEN
    ...
    RETURN jsonb_build_object(
      'result', 'not_found',
      'barcode', v_key,
      'message', 'Asset not registered'
    );
  END IF;

  SELECT id INTO v_existing_scan FROM inventory_scans
  WHERE session_id = p_session_id AND item_id = v_item.id AND result IN ('found', 'group_reconciled')
  LIMIT 1;
  ...
  IF v_item.is_group THEN
    ...
    LOOP
      ...
      SET found_quantity = LEAST(found_quantity + v_group_children.quantity, expected_quantity + 9999)
    ...
    UPDATE inventory_session_items
    SET found_quantity = LEAST(found_quantity + 1, expected_quantity + 9999)
    WHERE session_id = p_session_id AND item_id = v_item.id;
